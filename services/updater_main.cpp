...
static void GetAllPkgPeogress(UpdaterParams &upParams, std::string nowPkgLocation)
{
    size_t allPkgSize = 0;
    ...
    PkgManager::PkgManagerPtr manager)
{
    ...
    for (; upParams.pkgLocation < upParams.updatePackage.size(); upParams.pkgLocation++) {
        GetAllPkgPeogress(upParams, upParams.updatePackage[upParams.pkgLocation]);
        status = InstallUpdaterPackage(upParams, args, manager);
        ProgressSmoothHandler(static_cast<int>((upParams.initialProgress + upParams.currentPercentage) * 100));
        if (status != UPDATE_SUCCESS) {
