...
#define __UPDATER_LOG_FILE_NAME__   (strrchr((__FILE__), '/') ? strrchr((__FILE__), '/') + 1 : (__FILE__))
#define LOG(level) UpdaterLogger(level).OutputUpdaterLog((__UPDATER_LOG_FILE_NAME__), (__LINE__))
#define STAGE(stage) StageLogger(stage).OutputUpdaterStage()
#define ERROR_CODE(code) ErrorCode(code).OutputErrorCode((__UPDATER_LOG_FILE_NAME__), (__LINE__), (code))
