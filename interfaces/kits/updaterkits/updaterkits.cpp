...
bool RebootAndSecureErase(const std::string &eraseType, const std::string &cmd)
{
    ...
    int ret = snprintf_s(msg.update, sizeof(msg.update), sizeof(msg.update) - 1, "%s", secureEraseCmd.c_str());
    if (ret < 0) {
...
bool RebootAndSecureErase(const std::string &eraseType)
{
    ...
    struct UpdateMessage msg {};
    std::string secureEraseCmd = eraseCmd;
    int ret = snprintf_s(msg.update, sizeof(msg.update), sizeof(msg.update) - 1, secureEraseCmd.c_str());
    if (ret < 0) {
        LOG(ERROR) << "updaterkits: copy secure erase cmd message failed";
        return false;
    }
...
uint32_t EstimatedEraseTime(const std::string &eraseType)
{
    ...
    } else if (bootdevType & (1 << 2)) { // 2 ：means ssd / ssd composite device
